...
// SuperCx
bool WordList::InListNoCase(const char *s, const char*& word) {
	if (0 == words)
		return false;
	if (!sorted) {
		sorted = true;
	...
	if (j >= 0) {
		while (words[j][0] == firstChar) {
			if (MakeUpperCase(s[1]) == MakeUpperCase(words[j][1])) {
				const char *a = words[j] + 1;
				const char *b = s + 1;

				while (*a && MakeUpperCase(*a) == MakeUpperCase(*b)) {
					a++;
					b++;
				}
				if (!*a && !*b)
	...
	firstChar = MakeUpperCase(firstChar);
	j = starts[firstChar];
	if (j >= 0) {
		while (words[j][0] == firstChar) {
			if (MakeUpperCase(s[1]) == MakeUpperCase(words[j][1])) {
