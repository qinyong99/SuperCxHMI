	...
	for (int seg = 0; seg<maxEnd; seg++) {
		...
		if (endSeg > startSeg) {
			if (s[startSeg] <= '\002') {
				...
				xEnd = x + surface->WidthText(highlight ? fontBold : font, s+startSeg, endSeg - startSeg);
				if (draw) {
					rcClient.left = x;
					rcClient.right = xEnd;
					surface->DrawTextNoClip(rcClient, highlight ? fontBold : font, ytext,
...
//	startHighlight = 0; // SuperCx
//	endHighlight = 0;  // SuperCx
	inCallTipMode = true;
	posStartCallTip = pos;
	int deviceHeight = surfaceMeasure->DeviceHeightFont(size);
	font.Create(faceName, SC_CHARSET_DEFAULT, deviceHeight, false, false);
	fontBold.Create(faceName, SC_CHARSET_DEFAULT, deviceHeight, true, false); // SuperCx
