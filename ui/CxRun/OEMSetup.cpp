...
CString COEMSetup::GetModuleString(LPCTSTR lpszEntry, LPCTSTR lpszDefault)
{
...
BOOL COEMSetup::GetCreateServerObjectAsLocal()
{
	CString strValue = GetModuleString(_T("CreateServerObjectAsLocal"), _T("0"));
	int nValue = atoi(strValue);
	return nValue != 0;
}

BOOL COEMSetup::GetLocalServerUseSACProxy()
{
	CString strValue = GetModuleString(_T("LocalServerUseSACProxy"), _T("0"));
	int nValue = atoi(strValue);
	return nValue != 0;
}
