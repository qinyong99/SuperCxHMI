...
public:
	...
	int Show(int x, int y, int cx, int nSel, CEdit* pEdit = NULL)
	{		
		if (pEdit != NULL && pEdit->IsWindowVisible())
			m_pInplaceEdit = pEdit;
		...
		while (!m_bClose)
		{
			...
			if (msg.message >= WM_KEYFIRST && msg.message <= WM_KEYLAST)
			{
				...
				if (m_pInplaceEdit == NULL || nVirtKey == VK_UP || nVirtKey == VK_DOWN)
				{
					msg.hwnd = m_hWnd;
				}
				else
				{
					...
					msg.hwnd = m_pInplaceEdit->m_hWnd;
					SetCurSel(-1);
				}
			}
			else if (msg.message >= WM_MOUSEFIRST || msg.message <= WM_MOUSELAST)
			{
				...
				{
					CRect rcWindow;
					m_pInplaceEdit->GetWindowRect(&rcWindow);
					if ((msg.message != WM_LBUTTONDOWN && msg.message != WM_LBUTTONUP && msg.message != WM_RBUTTONUP) 
						|| rcWindow.PtInRect(msg.pt))
						msg.hwnd = m_pInplaceEdit->m_hWnd;
					else 
						msg.hwnd = m_hWnd;
				}
	...
	LRESULT OnKeyDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if (m_pInplaceEdit != NULL && GetCurSel() != nSel)
		{
			CString strText;
			...
			if (nSel != LB_ERR)
			{
				...
				m_pInplaceEdit->SetWindowText(strText);
				m_pInplaceEdit->SetSelAll();
			}
		}
