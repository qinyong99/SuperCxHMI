...
BEGIN_DISPATCH_MAP(CReportObj, CFormObj)
	...
	DISP_FUNCTION_ID(CFormDoc, "PrintOut", 1, PrintOut, VT_EMPTY, VTS_NONE)
	DISP_FUNCTION_ID(CFormDoc, "PrintPreview", 2, PrintPreview, VT_EMPTY, VTS_NONE)
	//}}AFX_DISPATCH_MAP
END_DISPATCH_MAP()
...
//	return theApp.m_pExtTypeLib->GetTypeInfoOfGuid(__uuidof(ReportObject), ppTypeInfo);
	return pThis->GetTypeInfoOfGuid(GetUserDefaultLCID(), __uuidof(ReportObject), ppTypeInfo);
}
...
void CReportObj::PrintOut()
{

}

void CReportObj::PrintPreview()
{
	ASSERT(m_pFormDoc != NULL);

	BOOL bAutoDelete = m_pFormDoc->m_bAutoDelete;
	m_pFormDoc->m_bAutoDelete = FALSE;
	GetDevDoc()->m_pRunCtrl->PrintPreview(NULL, m_pFormDoc, RUNTIME_CLASS(CReportView));
	m_pFormDoc->m_bAutoDelete = bAutoDelete;
}
