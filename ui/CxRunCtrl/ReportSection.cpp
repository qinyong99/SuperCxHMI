...
BEGIN_DISPATCH_MAP(CReportSection, CCmdTarget)
	...
	DISP_PROPERTY_EX_ID(CReportSection, "Name", CXPROPID_Name, GetName, SetNotSupported, VT_BSTR)
...
//	return theApp.m_pExtTypeLib->GetTypeInfoOfGuid(__uuidof(ReportSection), ppTypeInfo);
	return pThis->GetTypeInfoOfGuid(GetUserDefaultLCID(), __uuidof(ReportSection), ppTypeInfo);
}
...
void CReportSection::SetHeight(float fNewValue)
{
	...
	m_fHeight = fNewValue;
	m_pDocument->ReCalcReportHeight();
