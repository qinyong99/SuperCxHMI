// DrawArc.cpp : Implementation of CDrawArc
#include "stdafx.h"
...
void CDrawArc::InitArcPath(Gdiplus::GraphicsPath** ppObjectPath,
		POINT* points)
{
	POINTF ptTemp[3];
	for (int i = 0; i < 3; i++)
	...
	InitArcPath(ppObjectPath, ptTemp);
}
...
HRESULT CDrawArc::DoDraw(HDC hdcDest)
{
	using namespace Gdiplus;
...
HRESULT CDrawArc::DoPrint(HDC hdcDest)
{
	using namespace Gdiplus;
...
HRESULT CDrawArc::DoLoad(DWORD dwVer, LPSTREAM pStm)
{
...
HRESULT CDrawArc::DoSave(DWORD dwVer, LPSTREAM pStm)
{
...
HRESULT CDrawArc::DrawTrackerHandleShape(HDC hdcDest, long nHandle, POINT point)
{
	using namespace Gdiplus;

	if (nHandle < 0)
		return ICxDrawObjectImpl<CDrawArc>::DrawTrackerHandleShape(hdcDest, nHandle, point);
	...
	InitArcPath(&pObjectPath, points);
	PathData pathData;
...
HRESULT CDrawArc::DrawTrackerShape(HDC hdcDest, POINT* points, int nPoint)
{
	using namespace Gdiplus;
	...
	{
		return ICxDrawObjectImpl<CDrawArc>::DrawTrackerShape(hdcDest, points, nPoint);
	}
...
STDMETHODIMP CDrawArc::MoveHandlesTo(POINT* pPoints, int nCount)
{
	ATLASSERT(nCount == 3);

	delete m_pObjectPath;
	InitArcPath(&m_pObjectPath, pPoints);
...
//		point[in] 新位置点坐标
/////////////////////////////////////////////////////////////////////////////////////////
STDMETHODIMP CDrawArc::MoveHandleTo(long nHandle, POINT point)
{
	using namespace Gdiplus;

	if (nHandle < 0)
	{
		return ICxDrawObjectImpl<CDrawArc>::MoveHandleTo(nHandle, point);
	}
	...
	delete [] pPoints;
	delete m_pObjectPath;
	InitArcPath(&m_pObjectPath, points);
	ReSetRotateBasePoint();

	return S_OK;
}

STDMETHODIMP CDrawArc::HitTest(POINT point, BOOL* pbHitTest)
{
	using namespace Gdiplus;
...
//功能: 获取对象手柄数目
//////////////////////////////////////////////////////////////////////////////////
STDMETHODIMP CDrawArc::GetHandleCount(long* pHandleCount)
{
	*pHandleCount = 3;
	return S_OK;
}
...
////////////////////////////////////////////////////////////////////////////////////////////
STDMETHODIMP CDrawArc::GetHandle(long nHandle, POINT* pHandlePoint)
{
	using namespace Gdiplus;

	if (nHandle < 0)
	{
		return ICxDrawObjectImpl<CDrawArc>::GetHandle(nHandle, pHandlePoint);
	}
...
//功能: 取手柄位置光标句柄
///////////////////////////////////////////////////////////////////////////////////
STDMETHODIMP CDrawArc::GetHandleCursor(long nHandle, HCURSOR* pHandleCursor)
{
	if (nHandle < 0)
	{
		return ICxDrawObjectImpl<CDrawArc>::GetHandleCursor(nHandle, pHandleCursor);
	}
	*pHandleCursor = LoadCursor(NULL, IDC_CROSS);
	return S_OK;
}

STDMETHODIMP CDrawArc::GetClassName(BSTR* pbstrClassName)
{
	*pbstrClassName = OLESTR("Arc");
	return S_OK;
}

STDMETHODIMP CDrawArc::get_StartX(long *pVal)
{
	POINT point;
	GetHandle(1, &point);
...
STDMETHODIMP CDrawArc::put_StartX(long newVal)
{
	using namespace Gdiplus;
...
STDMETHODIMP CDrawArc::get_StartY(long *pVal)
{
	POINT point;
	GetHandle(1, &point);
...
STDMETHODIMP CDrawArc::put_StartY(long newVal)
{
	using namespace Gdiplus;
...
STDMETHODIMP CDrawArc::get_EndX(long *pVal)
{
	POINT point;
	GetHandle(2, &point);
...
STDMETHODIMP CDrawArc::put_EndX(long newVal)
{
	using namespace Gdiplus;
...
STDMETHODIMP CDrawArc::get_EndY(long *pVal)
{
	POINT point;
	GetHandle(2, &point);
...
STDMETHODIMP CDrawArc::put_EndY(long newVal)
{
	using namespace Gdiplus;
...
STDMETHODIMP CDrawArc::get_MiddleX(long *pVal)
{
	POINT point;
	GetHandle(3, &point);
...
STDMETHODIMP CDrawArc::put_MiddleX(long newVal)
{
	using namespace Gdiplus;
...
STDMETHODIMP CDrawArc::get_MiddleY(long *pVal)
{
	POINT point;
	GetHandle(3, &point);
...
STDMETHODIMP CDrawArc::put_MiddleY(long newVal)
{
	using namespace Gdiplus;
